build:
	mkdir -p bin

	nasm -f bin src/boot/bootloader.s -o bin/bootloader.bin
	nasm -f elf src/boot/kernel_entry.s -g -o bin/kernel_entry.o
	nasm -f elf lib/idt/IDT_handler_stubs.s -g -o bin/IDT_handler_stub.o

	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -O0 -c -o bin/kernel.o src/kernel/kernel.c
	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -O0 -c -o bin/screen_services.o lib/screen/screen_services.c
	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -O0 -c -o bin/stdlib.o lib/stdlib/stdlib.c
	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -O0 -c -o bin/IDT_handlers.o lib/idt/IDT_handlers.c
	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -O0 -c -o bin/string.o lib/string/string.c
	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -O0 -c -o bin/io.o lib/io/io.c
	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -O0 -c -o bin/pic.o lib/io/pic.c
	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -O0 -c -o bin/pmm.o lib/pmm/pmm.c
	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -O0 -c -o bin/gdt.o lib/gdt/gdt.c

	i686-elf-gcc -ffreestanding -fno-builtin -nostdlib -g -T src/linker.ld -o bin/kernel.elf bin/kernel_entry.o bin/kernel.o bin/screen_services.o bin/stdlib.o bin/IDT_handlers.o bin/string.o bin/IDT_handler_stub.o bin/io.o bin/pic.o bin/pmm.o bin/gdt.o

	i686-elf-objcopy -O binary bin/kernel.elf bin/kernel.bin

	cat bin/bootloader.bin bin/kernel.bin > bin/os.img
	truncate -s 1048576 bin/os.img

all: build
	qemu-system-i386 -drive id=disk,file=bin/os.img,format=raw,if=ide -boot order=c

debug: build
	qemu-system-i386 -drive id=disk,file=bin/os.img,format=raw,if=ide -boot order=c -s -S

clean:
	rm bin/*